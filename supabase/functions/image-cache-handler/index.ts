...
serve(async (req) => {
  ...
  try {
    ...
    const searchUrl = `https://www.googleapis.com/customsearch/v1?key=${googleApiKey}&cx=${googleCseId}&q=${encodeURIComponent(searchQuery)}&searchType=image&num=5`;
    ...
    // Try each result until we find one that's downloadable
    ...
    for (const item of googleData.items) {
      try {
        ...
        console.log('⚠️ Image download failed, trying next:', item.link);
        continue;
      }
    }
