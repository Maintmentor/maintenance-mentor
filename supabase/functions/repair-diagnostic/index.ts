...
🚫 RESTRICTIONS: Only answer repair/maintenance questions. Stay helpful and visual.`
        ...
        try {
          ...
          if (imageResponse.ok) {
            ...
            if (imageData.success && imageData.image) {
              ...
              // Try to get multiple images for better results
              ...
              for (const additionalQuery of additionalQueries.slice(0, 2 - partImages.length)) {
                try {
                  ...
                  if (additionalResponse.ok) {
                    ...
                    if (additionalData.success && additionalData.image && additionalData.image !== imageData.image) {
                      partImages.push({
                        query: partQuery,
    ...
    // Filter out any images that failed validation
    const validatedPartImages: typeof partImages = [];
    for (const img of partImages) {
      try {
        const headCheck = await fetch(img.url, {
          method: 'HEAD',
          signal: AbortSignal.timeout(5000),
        });
        if (headCheck.ok) {
          validatedPartImages.push(img);
        } else {
          console.log('⚠️ Removing inaccessible image from response:', img.url, headCheck.status);
        }
      } catch (err) {
        console.log('⚠️ Removing unreachable image from response:', img.url);
      }
    }
    console.log(`📊 Final response - Real part images found: ${partImages.length}, validated: ${validatedPartImages.length}`);
    ...
    return new Response(JSON.stringify({
      ...
      partImages: validatedPartImages,
      stepImages: [],
      videos: [],
      topic: 'general'
    }), {
