...
export default function ChatInterface({ conversationId, initialMessage }: any) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const isKeyMissing = typeof errorBody === 'string' && (
        errorBody.includes('API key not configured') || 
        errorBody.includes('OPENAI_API_KEY')
      );
      const isTimeout = typeof errorBody === 'string' && errorBody.includes('timeout');
      
      let errorMessage: string;
      let chatErrorContent: string;
      
      if (isKeyMissing) {
        errorMessage = 'AI service is not configured yet. An admin needs to add the OpenAI API key.';
        chatErrorContent = "The AI assistant is not configured yet. The OpenAI API key needs to be added to the server settings. Please contact your administrator or go to Admin Dashboard → API Keys to set this up.";
      } else if (isTimeout) {
        errorMessage = 'Request timed out. Please try again with a shorter question.';
        chatErrorContent = "I'm taking too long to respond. Please try again with a shorter or simpler question.";
      } else {
        errorMessage = 'AI service temporarily unavailable. Please try again.';
        chatErrorContent = "I'm having trouble responding right now. Please try again in a moment.";
      }
                          ...
                          <div key={idx} className="border-t pt-3">
                            ...
                            <img 
                              ...
                              onError={(e) => {
                                ...
                                errorDiv.textContent = `Failed to load image from ${part.source}`;
