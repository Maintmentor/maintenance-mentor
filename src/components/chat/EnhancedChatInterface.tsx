            ...
            <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className="max-w-[85%]">
                ...
                <Card className={`p-4 ${msg.sender === 'user' ? 'bg-gradient-to-br from-blue-500 to-blue-600 text-white' : 'bg-white'}`}>
                  ...
                  {/* No images warning - only show when user explicitly asked for images and the response isn't an error */}
                  {msg.sender === 'ai' && !msg.generatedImage && (!msg.partImages || msg.partImages.length === 0) && !msg.content.startsWith('⚠️') && (() => {
                    // Find the user message that preceded this AI message
                    const msgIndex = messages.indexOf(msg);
                    const prevUserMsg = msgIndex > 0 ? messages.slice(0, msgIndex).reverse().find(m => m.sender === 'user') : null;
                    const userText = (prevUserMsg?.content || '').toLowerCase();
                    const imageKeywords = [
                      'picture', 'photo', 'image', 'show me', 'what does', 'look like',
                      'looks like', 'identify', 'diagram', 'visual', 'see a', 'see the',
                      'show a', 'show the', 'photograph', 'illustration'
                    ];
                    const userAskedForImage = imageKeywords.some(kw => userText.includes(kw));
                    return userAskedForImage;
                  })() && (
                    <div className="mt-3 flex items-center gap-2 text-amber-600 text-sm">
                      <AlertCircle className="w-4 h-4" />
                      <span>Images couldn't be loaded right now. The AI response above still contains helpful information.</span>
                    </div>
                  )}
